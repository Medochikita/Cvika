...
serial.setSimulatedInputEvents([
    ...
    (timedelta(seconds=8.0), 0x39),
    (timedelta(seconds=8.1), 243),
    (timedelta(seconds=8.2), 254)
])
...
while not done:
    ...
    if len(nextByte) == 1:
        ...
        if len(pkt) == 3:  # We got the whole 3 byte packet, start processing it ...
            ...
            xSA4 = int((uint16(pkt[0] & 0x10) << 4) | uint16(x))      # TODO: Modify this line to solve self-assessment assignment 11 for 4th lecture
            print(", x=", x, ", x(SA4)=", xSA4, end="")

            y = pkt[2]
            if pkt[0] & 0x20 != 0:
                y |= 0x100

            ySA4 = int((uint16(pkt[0] & 0x20) << 3) | uint16(y))    # TODO: Modify this line to solve self-assessment assignment 11 for 4th lecture
            print(", y=", y, ", y(SA4)=", ySA4, end="")
